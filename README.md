...
| [mode-orchestrator](skills/mode-orchestrator/) | CC | — | Read a document holding a todolist + context, then run each step as an isolated `general-purpose` subagent turn with a role-mode `mode:`/`role:` header — one mode (and optional role) per turn, never mixed; autonomous modes only. Supports a per-turn model override, a bounded `failed`→`debug`→re-execute recovery loop, and per-task-type workflow specs (ships with `dev`) |
