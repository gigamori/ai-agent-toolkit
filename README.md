...
| [xml-wf](skills/xml-wf/) | CC | [docs](docs/skills/xml-wf/) | Build, run, and resume XML v2 workflows: a task is decomposed into single-responsibility steps orchestrated deterministically by a bundled Python runner (`wfrun`) — not by an LLM. In run-cc mode each step executes as an isolated `claude -p` subagent under an explicit role and mode; the run-llm protocol instead delegates steps through the hosting agent's own subagent facility, and `ask=` judgments auto-select the claude or pi CLI by harness |
...
│       ├── mode-orchestrator/ mode-orchestrator user guide (EN/JA) + workflow spec authoring contract
