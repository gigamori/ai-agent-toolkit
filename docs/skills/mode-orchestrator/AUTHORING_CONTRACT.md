...
**A change to `SKILL.md`'s run-directory record schema is a change to shared control flow.** Re-sweep BOTH `references/harness-*.md` **and `SKILL.md` itself** for instructions that tell the orchestrator to record something in the run index without naming a record, field or section, and give every such site an explicit disposition. `SKILL.md` carries several of its own — in Step -1 and in the Execution steps — and a sweep that reads only the harness references misses them. Judged 2026-08-28 and neither reference needed an edit: every site is settled by the schema's routing sentence (a recording the table has no field for goes in prose above the block) — the CC `deny_scan` verdict, and the Pi P0 resolutions and aborted-session path, all belong there. Confirmed in the first post-change run on each facet, which put them in prose with no extra key on any record.
...
The implementations are independent (this skill enforces the contract as prompt contract in `SKILL.md`; xml-wf enforces it in `wfrun` code) and some divergences are **deliberate** — e.g. this skill permits a scoped read of a turn's `## Decision request` section while xml-wf's run-llm orchestrator never reads a request body; this skill has amend-plan, xml-wf has continuation forms (a)/(b); this skill's run-index records vs xml-wf's ledgers (`events.jsonl`, the `decisions/` verdict markers). Do not "fix" those toward each other.

**The `decision` record's shape sits inside that ledger divergence — judged 2026-08-28, no action taken on either side.** `SKILL.md`'s run-directory record table pins the `decision` record as `key`, `parent`, `decider`, `file`, written on the `--decider=human` path only. That path exclusivity was reviewed against xml-wf and is **correct, not a drift to repair**: under `--decider=human` no turn is delegated, so without this record the adjudication would have no machine-readable trace at all, while under `llm` the inserted `decision-turn` already carries one in its own `turn` and `attempt` records. Two consequences a future session should not mistake for defects:

- **A consequence derived here, not a rule `SKILL.md` states:** because the record is produced on no other path, its `decider` field carries no discriminating value — it can only ever read `human`. Read it as self-description, and do not "generalize" the record to the `llm` path merely to give the field a second value.
- **A reader wanting every adjudication must union `decision` records with `turn` records of `kind: decision-turn`,** and take the decider from that kind, since no field on a turn or attempt record carries one. xml-wf answers the same question from a single record set filtered on a `decider` field, because its cap tally (`decision.llm_adjudications`) counts `llm` verdict markers and therefore requires them to exist. This skill's cap counts inserted turns instead. Neither side can adopt the other's shape without moving its cap, which is why the difference stands.

**Resolved 2026-08-28 — this skill now carries the escalate-when-unsure half of the escalation pin, which it previously lacked.** The inserted-decision-turn clause in `SKILL.md` names the three grounds (irreversible / outward-facing / changes what the run is for) and stops. xml-wf carries an additional standing instruction to escalate whenever the adjudicator is uncertain (`guardrails.py`'s `DECIDE_ROLE` and `DECIDE_PROMPT_TEXT`, plus its `run-cc.md` / `run-llm.md`). The registry pins that half as part of the escalation grounds, so the two sides currently differ on a pinned item. It is pre-existing and was not introduced by the record-shape change. Ruled rather than inherited: the clause was added to the inserted-decision-turn text, because `SKILL.md`'s own following sentence says the escalation clause is what stops an `llm` decider collapsing into deciding everything, and the missing half guards that same failure. Declaring the asymmetry deliberate would have needed a reason nobody had.
