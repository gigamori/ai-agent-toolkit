...
Operations resolve the active wiki by **existence**, top-down: **prompt `--root` > pj (`_projects/<project>/wiki/`) > workspace (`_llm-wiki/`) > CWD `.llmwiki` > child (exactly one immediate child directory with `.llmwiki`)**. The child scope is the last resort and is depth-1, never recursive: it rescues the common accident of opening the **parent** of a wiki folder. Two or more marked children is ambiguous and resolves to nothing (fail-closed) rather than silently picking one. The pj scope is read one-way from taskflow, resolved against `$TASKFLOW_PROJECT_ROOTS`: the marker hook (and the `resolve-root --sid` CLI flag) passes the turn's `session_id`, so the pj scope reads **only this session's own** `_projects/_state/<session_id>.json` `project` field — two sessions on different projects never resolve each other's wiki, even running concurrently. When that file is absent / unreadable / has no usable `project`, the pj scope is **skipped** (fail-closed, 2026-08-19) rather than falling back to another session's state. Only with **no** `session_id` at all (legacy callers) does the pj scope fall back to the most recent `_projects/_state/*.json` (mtime); with no state file at all the pj scope is skipped cleanly. Because resolution no longer depends on the CWD alone, the operations work in the **VSCode extension without `cd`**. The marker hook shows `active wiki: <root> (scope: pj|workspace|cwd|child)` each turn so the resolved wiki is always visible.
...
Operations resolve the active wiki multi-scope (prompt>pj>workspace>cwd>child), so they work without `cd` into the wiki; pass `--root <path>` to target a specific wiki explicitly.
...
`activation_scope: scoped` is implemented as a `UserPromptSubmit` hook (`hooks/wiki_marker_inject.py`): each turn it resolves the active wiki multi-scope (prompt>pj>workspace>cwd>child, via `wiki_root_resolver`), passing the turn's `session_id` so the pj scope reads this session's own state file only — with no fall back to a mtime-latest scan of another session's state (see *Resolving the active wiki*). When one resolves, it injects a `wiki-active` context (including the resolved root + scope, so the wiki is visible even when the CWD is not it — e.g. in the VSCode extension), the `[wiki:on]` leading-line directive, and — pj scope only — the wiki↔taskflow split/filing guide. When no wiki resolves in any scope it exits silently and injects nothing — outside a wiki, the plugin is invisible. The injected context plus the `wiki-query` skill description is what auto-activates query; the write-bearing operations are explicit commands and do not depend on the hook.
