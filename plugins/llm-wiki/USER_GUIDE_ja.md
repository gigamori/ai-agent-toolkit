...
取り違えることはありません。taskflow プロジェクトが割り当てられていないセッションには
pj wiki がそもそも存在しません — 他セッションの wiki を借用するのではなく、workspace や
current folder の wiki にフォールスルーする（あるいは何も解決しない）だけです。
