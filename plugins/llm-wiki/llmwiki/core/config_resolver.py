...
DEFAULTS: dict[str, str] = {
    ...
    "write_mode": "explicit",
    "write_autocommit": "auto",         # INERT: the engine invokes no git; kept for
                                        # config stability (removing would churn AXES).
    "override_scope": "operation",
    "apply_fanout_k": "10",
    "max_count": "100",
    "max_bytes": "10485760",
    # optional qmd search backend (opt-in; optional-search-qmd.md D-Q7). Independent
    # of max_count — no check_consistency coupling is added (D-Q7). search_backend
    # defaults to "index" so the backend is off unless a wiki opts in.
    "search_backend": "index",          # index | qmd — selects the query read path
    "qmd_bin": "qmd",                   # binary resolved via PATH (shutil.which)
    "qmd_page_threshold": "100",        # int-as-str; qmd used only when len(scan_pages) > this
}
...
def autocommit_forced(resolutions: dict[str, Resolution]) -> bool:
    ...
    INERT: the engine invokes no git, so nothing acts on this. Retained for
    config/API stability; kept pure so callers/tests referencing it still resolve.
    """
    return resolutions["write_mode"].value == "implicit"
