...
I/O contract:
    ...
    WriteSession(wiki_root, *, max_count, max_bytes, origin)
      ...
      .commit() -> list[str]
        writes every staged op to disk (caller runs this inside the file-journal
        transaction; each target is journaled before the write); returns the
        written rel_paths.
...
class WriteSession:
    ...
    def commit(self) -> list[str]:
        # Record each target's pre-image in the transaction journal BEFORE writing
        # (no-op if no journal dir — i.e. not inside a transaction). Auto-journals
        # so the cross-process ingest apply path (separate `ingest-apply` process
        # under the orchestrator's lock) is still rolled back on a failed finish.
        from llmwiki.write import transaction
        transaction.journal_before_write(
            self.wiki_root, [op.rel_path for op in self.ops])
