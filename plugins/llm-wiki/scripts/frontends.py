...
I/O contract (each front-end):
    ...
    fe_b_prime(wiki_root, markdown) -> FEResult      # markdown from extract_cc_log
...
The front-ends DO NOT write to disk by themselves and DO NOT commit; the single
git transaction (transaction.py) orchestrates lock/checkpoint/write/commit. A
front-end only computes the artifact (redact -> hash -> dedup-check -> assemble).
...
def fe_b_prime(wiki_root: "str | Path", markdown: str) -> FEResult:
    ...
    `markdown` is the output of extract_cc_log.extract_markdown (text + tool_use).
