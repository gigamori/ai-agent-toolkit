...
split on a tab, and do NOT let `$WIKI_SCOPE` come out empty: Step 2 passes it to
the driver as `--scope`, and an empty value silently narrows a workspace/pj-scoped
run to the cwd-slug session set. If it exits non-zero (`NO-WIKI`), no wiki resolved — report
...
`active wiki: <root> (scope: pj|workspace|cwd|child|prompt)`). The driver still enforces the
...
- **Failure-continue:** if any step for a sid fails, roll back just that sid —
  ...
  Route by where it failed (Step 8): a `begin` error → `begin` already rolled back and
  released the lock itself, so do not call `finish` — report its stderr only; a
  Stage1/Stage2 error, or a `plan-fanout` budget gate (a failure before apply) → call
  its own `finish fail` (Step 8); an `apply-finish` REJECT (a write gate or an F2
  ordinal/page-set mismatch) → `apply-finish` has already rolled that sid back, so do not
  also call `finish`. One session's failure must not stop the batch (partial success is
  allowed; F3 makes the shared prefix survive on the next sid).
...
code-authored absolute path to save cluster `i`'s manifest to (Step 7) — one per ordinal,
aligned to `clusters`. Always call it: the 0-based index of each cluster in the returned
list is that cluster's ordinal. `apply-finish` (Step 8) proves every cluster was dispatched
...
from `plan-fanout`'s stdout — do not construct a temp path yourself; it lands under
`$OUT_DIR`, so it rides the `project-batch-cleanup` sweep — no new temp mechanism, E2/F3).
...
still open, so roll it back explicitly. (A `begin` error is NOT routed here — see Step 4:
`begin` already rolled back and released the lock itself on failure, so report its stderr
only and do NOT call `finish` for a `begin` error.)
