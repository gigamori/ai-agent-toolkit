...
Owner judgement — a one-line summary per active task and note↔task links for freshly-written `project-notes/` deliverables — is delegated to the async `taskflow:progress-capture` subagent: the hook commits `capture.status=requested` (freezing the round's task set) and blocks once with an instruction to spawn it; the subagent writes a `{session_id}.capture` JSON sidecar; a later `Stop` applies it deterministically (`@log` summaries via `append_auto_binding`, note links via `append_note_link`). If no sidecar appears within a 30 s expiry, a deterministic backstop placeholder-binds every task of that round that has not produced a line yet. A subagent that finishes *after* that expiry is not wasted: the round's task/note set is retained until the next round is requested, so a late sidecar still applies its summary and its note links on top of the placeholder. Placeholder lines carry an `(r<N>)` round tag, which is also the idempotency key — re-applying an identical entry never duplicates a line. Round / lifecycle state (cursor, round number, per-task line counts) lives in a `{session_id}.bind` sidecar (kept separate from the state JSON so concurrent rewrites cannot clobber it); the old `{session_id}.captured` marker is legacy and only swept by the 7-day cleanup. See `_projects/harness-taskflow/project-notes/specs/exec-binding.md` (development-repo design notes; not shipped with the plugin) and `project-notes/specs/note-task-link.md` for the design.
