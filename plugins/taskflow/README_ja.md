...
owner 判定 — active task ごとの 1 行 summary と、新規書込された `project-notes/` 成果物の note↔task link — は async な `taskflow:progress-capture` サブエージェントに委譲する: hook は `capture.status=requested` を commit し（そのラウンドの task 集合を凍結する）、サブエージェント起動を促す block を 1 回返す。サブエージェントは `{session_id}.capture` JSON サイドカーを書き、後続の `Stop` がそれを決定論的に apply する（`@log` summary は `append_auto_binding`、note link は `append_note_link`）。30 秒の expiry 内にサイドカーが現れなければ、決定論バックストップが、そのラウンドでまだ 1 行も落ちていない task を placeholder-bind する。expiry の**後**に完了したサブエージェントも無駄にはならない: そのラウンドの task/note 集合は次ラウンドが要求されるまで保持されるため、遅れて届いたサイドカーも placeholder の上に summary と note link を apply する。placeholder 行には `(r<N>)` のラウンドタグが付き、これが冪等キーも兼ねる — 同一エントリの再 apply が行を重複させることはない。round / lifecycle 状態（カーソル・ラウンド番号・task ごとの行数）は `{session_id}.bind` サイドカーに保持する（state JSON とは分離し、他フックの並行書換による clobber を防ぐ）。旧 `{session_id}.captured` マーカーは legacy で、7 日クリーンアップで掃除されるのみ。設計は `_projects/harness-taskflow/project-notes/specs/exec-binding.md`（開発リポジトリの設計メモ。プラグインには同梱されない）と `project-notes/specs/note-task-link.md` を参照。
