...
- **タスクファイルに触るのはメインエージェントだけ。** オーケストレーションされたターン（サブエージェント）は書き込まない。`@log` のタイムスタンプとセッション ID は、注入される Progress Session コンテキストの `iso_ts=` / `sid=` から逐語コピーする — 手で計算しない。
...
- **中位以上のモデルで駆動すること。** 弱いモデルは単に失敗するのではなく、**実在しない mode 名でもっともらしい計画を捏造して先へ進む** — 上のルールが禁じている模倣そのものだ。実測では、小さいモデルの 2 標本は正しい実行 0、一段上のモデルの 2 標本は 2 だった。
...
### notes に入れないもの: LLM 間の handoff

**別の Claude セッション**が受け取るために書く handoff — セッション間の申し送り、姉妹リポジトリへの依頼 — は永続知識ではなく一時的なメモだ。これらは `_projects/<project>/llm-handoff/` に置く: flat なディレクトリで、index も状態フォルダも持たず、`progress.md` とも連動しない。明示的に指す（`@llm-handoff/<filename>`）まで Claude のコンテキストには入らず、いつでも手で削除してよい — 削除前に何かを蒸留して残すこともしない。

**あなた自身が実行する** handoff — E2E / デバッグ handoff のように、テスターが人間で、記入済みの結果が残す価値を持つもの — は逆に永続的な記録なので `project-notes/checks/` に留まる。置き場を決めるのは「handoff」という語ではなく、誰がその文書を消費するかだ。
