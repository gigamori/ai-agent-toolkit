...
| Path | Resolution |
|---|---|
| `_projects/` (every root-resolving hook except `touched_capture.py`; the payload-derived hooks resolve no root — see the layer table below) | `os.getcwd() + '/_projects'` (CWD-based) |
| `_projects/` (`touched_capture.py` only) | `STATE_ROOT + '/_projects'`, where `STATE_ROOT = _find_state_root(os.getcwd()) or os.getcwd()` — the nearest ancestor of the cwd (the cwd itself included) that holds `_projects/_state`, falling back to the cwd when no ancestor qualifies. The same `STATE_ROOT` is the base `main()` normalizes recorded paths against (`normalize_path(p, cwd)` is called with `cwd = STATE_ROOT`), so the ledger location and the relative-path base always move together. See the hook-exception row under "Hooks (CWD-fixed) vs. the command / viewer layer" below. |
...
|---|---|---|
| **Hooks** (`session_init.py`, `session_sync.py`, `session_compact_reset.py`, `session_progress_capture.py`, and `precompact_flush.py`, which imports `PROGRESS_ROOT` / `STATE_DIR` from the latter) | `os.getcwd()` — CWD-fixed, always the `_projects/` under the session's launch cwd | A hook process inherits the cwd the session was **launched** in, and that value is fixed for the session's whole life — a `cd` inside a Bash tool call does not reach hook processes. Keeping these hooks CWD-pure removes any cross-workspace ambiguity mid-session and needs no configuration. What it does **not** buy is a guarantee that the cwd is the workspace root: a session launched from a subdirectory resolves `_projects/` to a path that does not exist, and each of these hooks then no-ops for that entire session. |
| **Hook exception** (`touched_capture.py`) | `_find_state_root(os.getcwd()) or os.getcwd()` — nearest ancestor (cwd included) holding `_projects/_state` | The PostToolUse `.touched` ledger is the sole input to task and note resolution, so under a subdirectory launch the silent no-op above costs the whole session's writes their membership — with no diagnostic. This hook therefore re-anchors on the tree that actually holds the state, and normalizes recorded paths against that same `STATE_ROOT`. It is append-only and runs no sweep, which is why the ancestor walk was applied here first: putting `session_progress_capture.py`'s unconditional `_cleanup_stale_markers` sweep behind the same walk is a different risk class and is deferred to a separate task. An unrelated tree that happens to hold `_projects/_state` is reachable by the walk, but the `<session_id>.json` orphan guard in `main()` stops a foreign ledger from being written. |
| **Payload-derived** (`task_rebuild_progress.py`, `notes_index_reminder.py`) | the `_projects/<project>/` prefix of the written path in the hook's own payload — `os.getcwd()` is never read | These two act only when the payload text itself already names `_projects/<project>/...` (a `re.search` over the written path, resp. over the Bash command), so the root travels with the payload. They are immune to the launch-cwd failure mode above, which is why neither belongs in the CWD-fixed row. |
...
The premise this split was originally argued from — that a hook "fires inside one Claude Code session whose CWD *is* the workspace" — does not hold, and the exception row above is what falsified it. Measured on Claude Code 2.1.233 (2026-08-19, three-arm probe): hook cwd is the cwd the session was launched in and nothing moves it afterwards (a same-session `cd` in a Bash tool call leaves `os.getcwd()` unchanged in the PostToolUse process), but a session launched in `<ws>/sub` reports `<ws>/sub` — and reports it identically in `os.getcwd()`, in `CLAUDE_PROJECT_DIR`, and in the hook payload's `cwd` field, so neither of the latter two is a better anchor than the cwd itself. `touched_capture.py` is the one hook that no longer rests on the premise; the others still do, and their failure mode under a subdirectory launch is a silent whole-session no-op rather than a wrong target.
