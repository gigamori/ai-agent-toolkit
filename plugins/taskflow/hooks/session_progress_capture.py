...
def main() -> int:
    ...
    # The legacy un-suffixed `{sid}.capture` name is deliberately no longer
    # read here: nothing writes it any more (rounds only ever hand out the
    # per-round `{sid}.r{N}.capture` name, §4.4.1 D1), so there is no reader
    # branch to restore. `_CLEANUP_SUFFIXES` still ages a stray legacy file
    # out via the 7-day sweep.
