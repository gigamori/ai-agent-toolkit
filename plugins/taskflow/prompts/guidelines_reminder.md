...
RESPONSE LEADING LINES: when a project is assigned, ALWAYS include [pj:<project>] in the response's leading lines (near the beginning, before the main body; it may follow other leading lines such as [Mode:], not necessarily the first line). When unassigned, omit [pj:...] entirely.
