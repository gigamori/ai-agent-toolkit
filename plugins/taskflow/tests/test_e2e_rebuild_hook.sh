#!/usr/bin/env bash
# test_e2e_rebuild_hook.sh — E2E test via `claude -p`
#
# Sends natural-language prompts to Claude via CLI. The taskflow harness
# (guidelines + hooks) should autonomously create/move task files and
# the PostToolUse hook should auto-rebuild progress.md.
#
# Usage:  bash plugins/taskflow/tests/test_e2e_rebuild_hook.sh
...
# Common claude flags — skip permissions, use haiku for speed/cost
CLAUDE="claude -p --dangerously-skip-permissions --model haiku"

# System prompt that sets up the taskflow context for the sub-session
...
# ----------------------------------------------------------
# Scenario 1: Natural task creation
# ----------------------------------------------------------
...
# Check: task file exists somewhere in 0_todo
...
# ----------------------------------------------------------
# Scenario 2: Create a second task
# ----------------------------------------------------------
echo ""
...
# ----------------------------------------------------------
# Scenario 3: Move task to in_progress
# ----------------------------------------------------------
echo ""
echo "[Scenario 3] Ask to start work on Alpha task"

# Re-find alpha file (name may vary)
ALPHA_FILE=$(find "$PROJECT_DIR/tasks/0_todo" -name "*.md" 2>/dev/null | head -1)
