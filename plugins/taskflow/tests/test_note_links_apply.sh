...
if [ -z "$O9B" ] || ! echo "$O9B" | grep -q '"decision": *"block"'; then
 pass "Stop#2 (sidecar absent, <expiry) does NOT block → pending, no re-spawn"
  else
...
[ "$(sidlines "$T9")" = "0" ] && pass "pending state still does not bind" || fail "bound while pending"
  ...
  echo ""
echo "sidecar apply: note link established, real summary wins, idempotent"
reset_state
...
[ "$BEFORE_HASH" = "$AFTER_HASH" ] \
  && pass "writing the sidecar did NOT modify the task @log/@notes" \
...
[ "$(notecount "$T1" "$N1REL")" = "1" ] && pass "note link established in task @notes" || fail "note link not established"
[ ! -f "$(rcap 1)" ] && pass "sidecar consumed (unlinked) after apply" || fail "sidecar not consumed"
echo "$O1B" | grep -q "linked note: $N1REL -> $PROJ/$T1BASE" \
  && pass "F5 linked-note observability surfaced" || fail "no linked-note F5: $O1B"

O1C=$(stop 999)
[ "$(sidlines "$T1")" = "1" ] && pass "re-run leaves exactly one [s:] line (idempotent)" || fail "duplicate @log line"
[ "$(notecount "$T1" "$N1REL")" = "1" ] && pass "re-run leaves exactly one note link (idempotent)" || fail "duplicate note link"

  echo ""
echo "expiry: placeholder binds touched task, referenced over-binds known owner, unlinked note not established"
reset_state
