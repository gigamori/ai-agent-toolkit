...
if [ -z "$O22" ] || ! echo "$O22" | grep -q '"decision": *"block"'; then
  ...
  echo ""
echo "M-1 bootstrap: pre-D1 .bind does not replay history"
reset_state
T3="$PDIR/tasks/1_in_progress/2026-08-09_migrate.md"; mk "$T3"
write_touched "$T3"; write_touched "$T3"; write_touched "$T3"
cat > "$BF" << 'EOF'
...
[ "$(bindq 'c.get("round")')" = "1" ] \
  ...
  echo ""
echo "re-applying the identical capture summary adds no duplicate"
reset_state
...
echo "$O42" | grep -q "applied summary: $PROJ/2026-08-09_reapply.md" \
  ...
  || pass "the no-op re-apply is not re-reported"

  echo ""
echo "note write -> reverse-index owner enters A_r"
reset_state
