...
def test_env_set_uses_env_universe_only() -> None:
    with tempfile.TemporaryDirectory() as tmp:
        ...
        check(_synced(ws, "plans") == ["plan_env.md"],
              f"env set -> only env-universe plans are synced, got {_synced(ws, 'plans')}")
        check(_synced(ws, "memory") == ["mem_env.md"],
              f"env set -> only env-universe memory is synced, got {_synced(ws, 'memory')}")
...
def test_env_unset_uses_home() -> None:
    with tempfile.TemporaryDirectory() as tmp:
        ...
        check(_synced(ws, "plans") == ["plan_home.md"],
              f"env unset -> ~/.claude plans are synced, got {_synced(ws, 'plans')}")
        check(_synced(ws, "memory") == ["mem_home.md"],
              f"env unset -> ~/.claude memory is synced, got {_synced(ws, 'memory')}")
...
def test_env_relative_value_is_literal() -> None:
    ...
    with tempfile.TemporaryDirectory() as tmp:
        ...
        check(_synced(ws, "plans") == ["plan_literal.md"],
              f"'~/cfgtest' resolves to <cwd>/~/cfgtest, got {_synced(ws, 'plans')}")
        check(not (fake_home / "cfgtest").exists(),
              "'~/cfgtest' is not expanded to the home directory")
