#!/usr/bin/env bash
# test_task_rebuild_hook.sh — E2E test for PostToolUse auto-rebuild hook
#
# Creates a temporary project under _projects/, simulates task operations
# via the hook, and verifies progress.md is rebuilt correctly.
#
# Usage:  bash plugins/taskflow/tests/test_task_rebuild_hook.sh
...
# Convert MSYS path to Windows native with forward slashes (JSON-safe)
to_win() {
  if command -v cygpath >/dev/null 2>&1; then
    cygpath -m "$1"  # -m = mixed mode: forward slashes
  else
    echo "$1"
  fi
}
...
# ----------------------------------------------------------
# Setup: create project skeleton (no progress.md yet)
# ----------------------------------------------------------
...
# ----------------------------------------------------------
# Scenario 1: Create a task in 0_todo — progress.md should be created & populated
# ----------------------------------------------------------
...
# ----------------------------------------------------------
# Scenario 2: Add a second task — both should appear
# ----------------------------------------------------------
echo ""
...
# ----------------------------------------------------------
# Scenario 3: Move task to 1_in_progress, edit it — table updates
# ----------------------------------------------------------
echo ""
...
# Verify Feature A is NOT in TODO anymore
...
# ----------------------------------------------------------
# Scenario 4: Move task to 2_done — appears in Completed
# ----------------------------------------------------------
echo ""
...
# ----------------------------------------------------------
# Scenario 5: Non-task path — hook should be silent (no rebuild)
# ----------------------------------------------------------
echo ""
...
# ----------------------------------------------------------
# Scenario 6: Japanese slug in filename
# ----------------------------------------------------------
echo ""
