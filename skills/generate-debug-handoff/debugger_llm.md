...
1. fork directive を構築する:「継承文脈に inline 注入済の本 skill の Output Template / Rules に従い debugger:llm で handoff 本文を 1 パス生成せよ。文脈で確定できないセルは推測で埋めず `?` とし、不足項目は handoff 本文とは別に "Unresolved" として列挙して返せ（handoff の独立セクションにはしない）。Markdown のみ返せ（write 禁止）」。canary 値（会話既知の target 等）は directive に書かない
...
5. SKILL.md の Output Template に従い handoff 本文を生成し、Markdown のみ返す（write しない）
...
8. 保存先を user に確認する（slug 候補を自動提案）
9. handoff を書き出す
