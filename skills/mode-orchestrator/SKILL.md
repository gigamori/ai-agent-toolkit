...
- Artifacts: `NN-<mode>.md`; dynamic turns take the next suffix on their originating `NN`. `index.md` contains one JSONL block. The block holds exactly six record kinds — `contract`, `plan`, `turn`, `attempt`, `decision`, `amendment` — each carrying `record` plus exactly the fields this skill gives for it, no others. **Everything else this skill or a harness reference tells you to record in the run index goes in prose above the block**, never as a new record kind and never as an extra field on an existing one. A first-pass `turn` record has `kind: planned` and null parent/inherits; only a listed dynamic kind may have a non-null parent. `turn` records require `key`, `plan`, `kind`, `step`, `parent`, `inherits`, `inputs`, `mode`, `effort`, `source`, `model`, and `planned_override`. **`inputs` holds record KEYS (`["03a"]`), not paths** — paths are the prompt's view of the same handover (step 5). **`model` holds the resolved concrete model, never a tier** — it records the resolution, which is what an audit compares the overrides against. `attempt` records require `turn`, `attempt`, `actual_override`, `delegation_ref`, `reported_status`, `effective_status`, and `file`. `decision` records are keyed non-turns written only on the `--decider=human` path — the `llm` path's `decision-turn` records are its adjudication — and require `key`, `parent`, `decider`, and `file`. An amendment requires `key`, `plan`, `replaces`, `after_turn`, `parent`, `source_file`, `source_sha256`, complete ordered `supersedes`, and complete ordered `replacements`; each replacement has new key, parent amendment, and `amendment_item`. Reject duplicate/missing references, a partial tail, or an attempt for a superseded turn. This index is inspection evidence, not a resumable scheduler.
