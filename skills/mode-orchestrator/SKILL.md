...
- **Effort and model — precedence**: read the resolved harness column of `references/execution-profiles.md`.
  1. A todolist `model:` wins; record `effort: -`, source `step-model`.
  2. Otherwise a todolist `effort:` wins; then an active workflow's step effort; then infer effort from the final instruction. Use `middle` when unsure.
  3. Resolve `low` / `middle` / `high` from the profile and record source `step-effort` / `workflow-effort` / `inferred-effort`.

  Every autonomous turn has a resolved model and passes it as the delegation override. Never cross-fallback, guess a model, or use the harness default. Record effort, source, resolved model, and the override actually passed.

## Turn plan

Build an ordered list of turn records. Each record:

- `order`, `mode`, `role` (optional), `effort`, `model source`, `model`, `override`, `inputs` (file paths), `instruction`
...
Unless `--auto`, present the turn plan (order / mode / role / effort + source / model / override / one-line gist per turn), the spec warnings, and the Failure & decision policy block, then wait for approval before executing.
...
2. Delegate the turn per the harness reference's **P1 delegation method** (read in Step -1), passing the resolved `model` as that method's override and recording the value actually passed. One turn = one delegated call; never combine turns.
...
6. **Recovery loop** — when an `execute` turn returns `failed`:
   ...
   2. Insert a re-execute turn (`mode: execute`, effort/model/source = the failed turn's): apply the diff proposed in `NNa-debug.md` and re-run the original planned checks; deliverable `NNb-execute.md`.
   ...
   - The inserted `debug` turn resolves `high`; the re-execute turn keeps the failed turn's effort, model, and source.
...
- Artifacts: `NN-<mode>.md` in order; every turn inserted for an originating turn `NN` — by either loop — takes the next free suffix letter for that `NN`, in insertion order, preserving the originating order. The two loops share one letter sequence, so a turn that goes through both reads as e.g. `05a-decision.md` → `05b-execute.md` → (that re-run fails) → `05c-debug.md` → `05d-execute.md`. Because the mode is in the filename, nothing is ambiguous about which loop produced which artifact. Plus a small index file recording the turn plan. One turn line is `NN mode effort/source → model (override: model)`; it also records status/path, loop counts, continuation/amendment/wait records, and aborted re-runs with their detection path. This index is an artifact index for inspection, not a resumable scheduler.
