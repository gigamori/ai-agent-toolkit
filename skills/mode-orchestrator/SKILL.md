...
- `--decider=human|llm`: who adjudicates a `needs-decision` turn (see the decision loop, Execution step 7). Default: `llm` — the decision is delegated to an inserted `review-dev` turn. `human` stops at the step boundary and waits for the user's answer. This is a run-level setting; there is no per-point form, because a demand-driven mechanism has no points declared ahead of time.
...
That escalation clause is not optional wording: without it, a default `llm` decider collapses into deciding everything, which is exactly the failure the two-value `needs-decision` / `needs-human` split exists to prevent.
