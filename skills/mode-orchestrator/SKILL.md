...
After reading the selected harness reference, read `references/execution-profiles.md`. It is the authority for `basic` / `pro` / `ultra` mappings. A missing or malformed profile is `blocked` before delegation.
...
- **Model / effort**: read the selected harness column of `references/execution-profiles.md`.
  ...
  3. Then `(workflow-step: ID)`: exactly one active workflow row with that `ID` may pin `basic|pro|ultra`, source `workflow-effort`; `(infer)` is no pin. Duplicate IDs are blocked.
  4. Otherwise classify the final instruction as `basic`, `pro`, or `ultra`, source `inferred-effort`; use `pro` when unsure.
...
Build ordered turn definitions: key, plan id, kind, step origin, parent, inherits, inputs, mode, role, effort/source, model, and `planned_override`. Every initial turn has literal `kind: planned`—never its mode name. Dynamic kinds are only `recovery-debug`, `recovery-reexecute`, `decision-turn`, or `decision-rerun`. The tier is resolved to its bound model before dispatch: `planned_override` — and `actual_override` (Execution step 2) — always carry the bound concrete model the harness receives, never a tier name (`basic` / `pro` / `ultra`); the `effort` field is what carries the tier. `planned_override` is the exact model argument only (for example `haiku`), never an effort/model display such as `basic/haiku`. One mode and at most one role per record.
...
4. **A turn that reports nothing is `aborted` — infrastructure failure, not a task outcome.** Two paths reach it:
   ...
   **Late reply — the one case where a fired verdict does not mean `aborted`.** On a harness whose P2 runs as a process separate from the turn, the two can race: the verdict fires, and the turn's own reply arrives anyway a short time later. **A well-formed `status:` line arriving within the grace window the harness reference states — or meeting any further admissibility test that reference defines — classifies the turn, and the verdict is recorded as superseded rather than acted on.** Outside those, or with no reply, the first bullet stands and the turn is `aborted`. Record which applied — an undefined case that happened to resolve correctly is not a defined case, and the run index is where the distinction survives. This does **not** license waiting: you may only classify by a reply you already have when you come to classify, never delay classification hoping one arrives. Whether this case can arise at all is a property of the harness's mechanism, so the reference decides — where P2 is synchronous with the turn (the two outcomes are mutually exclusive by construction), the reference says so and this clause never fires there. Measured 2026-08-27 on Claude Code: a turn whose deliverable landed at 1393 s of a 1500 s budget replied `ok` at ~1546 s, after `TIMEOUT`; re-running it would have re-executed a non-idempotent `execute` turn over nine files to buy nothing.
5. **Chaining**: a later turn receives earlier artifacts by path and reads the full files itself — never forward a gist as the next turn's input.
...
6. **Recovery loop** — when an `execute` turn returns `failed`:
   1. Insert `recovery-debug`: parent = failed execute, inherits = `null`, inputs = plan plus its Failure report, effort/source = `ultra/policy-effort`; its deliverable is `NNa-debug.md`. Missing Failure report fields yield `needs-human`.
   ...
   - `recovery-debug` is `ultra/policy-effort`; `recovery-reexecute` preserves its `inherits` turn's effort, source, model, and planned override.
...
7. **Decision loop** — when any turn returns `needs-decision`. It is the recovery loop's twin: neither is declared in advance, both are opened by the turn's own status line, and both are bounded per originating turn — the decision bound counting inserted turns, so it applies to `--decider=llm` runs only (step 7.5).
   1. **`--decider=llm`** — insert `decision-turn`: parent = raising turn, inherits = `null`, inputs = raising deliverable, plan, and input document; mode `review-dev`, effort/source `ultra/policy-effort`. It validates the Decision request and returns `needs-human` if incomplete.
...
- Artifacts: `NN-<mode>.md`; dynamic turns take the next suffix on their originating `NN`. `index.md` contains one JSONL block. The block holds exactly six record kinds, each carrying `record` plus exactly the fields the table below gives it, no others. **Everything else this skill or a harness reference tells you to record in the run index goes in prose above the block**, never as a new record kind and never as an extra field on an existing one. A first-pass `turn` record has `kind: planned` and null parent/inherits; only a listed dynamic kind may have a non-null parent.

  | record | fields besides `record` |
  |---|---|
  | `contract` | `version` = `adaptive-effort-v1`; must be the block's first line |
  | `plan` | `id`, `replaces`, `after_turn` |
  | `turn` | `key`, `plan`, `kind`, `step`, `parent`, `inherits`, `inputs`, `mode`, `effort`, `source`, `model`, `planned_override`; `role` optional; `amendment_item` when `parent` is an amendment key |
  | `attempt` | `turn`, `attempt`, `actual_override`, `delegation_ref`, `reported_status`, `effective_status`, `file` |
  | `decision` | `key`, `parent` (the raising turn), `decider`, `file` — `--decider=human` only; under `llm` the adjudication is the `decision-turn`'s own records and none is written |
  | `amendment` | `key` (`amend-<n>`), `plan`, `replaces`, `after_turn`, `parent`, `source_file`, `source_sha256`, complete ordered `supersedes`, complete ordered `replacements`; each replacement has new key, parent amendment, and `amendment_item` |

  **`inputs` holds record KEYS (`["03a"]`), not paths** — paths are the prompt's view of the same handover (step 5). **`model` holds the resolved concrete model, never a tier** — it records the resolution, which is what an audit compares the overrides against. Reject duplicate/missing references, a partial tail, or an attempt for a superseded turn. This index is inspection evidence, not a resumable scheduler.
